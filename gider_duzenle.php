...
// Düzenlenecek gider kaydını seçme
if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // Mevcut gider bilgilerini çekme
    $stmt = $conn->prepare("SELECT aciklama, miktar, tarih FROM giderler WHERE id = ?");
    if ($stmt) {
        ...
        die("Gider bilgileri alınamadı: " . $conn->error);
    }
} else {
    die("Geçersiz gider ID'si.");
}

// Gider bilgilerini güncelleme
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aciklama'], $_POST['miktar'], $_POST['tarih'])) {
    $aciklama = $_POST['aciklama'];
    $miktar = $_POST['miktar'];
    $tarih = $_POST['tarih'];

    $stmt = $conn->prepare("UPDATE giderler SET aciklama = ?, miktar = ?, tarih = ? WHERE id = ?");
    if ($stmt) {
        ...
        die("Gider güncelleme hatası: " . $conn->error);
    }
}
?>
...
<body>
    <div class="content">
        <div class="container">
            <h2 class="mt-4 mb-3">Gider Düzenle</h2>
